  ...
  return (
    <div onClick={handleClickNotify}>
      <div className={`w-full flex items-center justify-start gap-3 p-4  cursor-pointer ${notify.isSeen ? "hover:bg-gray-50 dark:hover:bg-gray-600" : "bg-slate-50 hover:bg-slate-100 dark:bg-slate-600 dark:hover:bg-slate-700"}`}>
        <div className={`flex items-center justify-center w-10 h-10 rounded-full ${notify.type== "comment" ? "bg-blue-50 text-blue-500" : "bg-green-50 text-green-500"}  dark:bg-slate-700 dark:text-gray-200`}>
          {notify.type == "comment" ? <FaRegMessage className='size-4'/> : <IoPersonAddOutline className='size-4'/>}
        </div>
        <div className='w-[90%]'>
